...
line=0 
for ((j = iTrayWidth * iTrayHeight; j > 0; j -= iTrayWidth)) 
do 
...
echo -ne "\33[1m\33[3${iMap[$j]}m\33[4${iMap[$j]}m[]\33[0m" 
fi 
done 
done 
} 
...
sBox="[]" 
s=$s"\33[1m\33[3${cBoxCur}m\33[4${cBoxCur}m" 
fi 
...
#display new box 
echo -ne "\33[1m\33[3${cBoxNew}m\33[4${cBoxNew}m" 
for ((j = 0; j < 8; j += 2)) 
do 
((i = iTop + 1 + ${boxNew[$j]})) 
